...
go mod tidy
#build linux binary
GOOS=linux GOARCH=amd64 GIN_MODE=release go build -o bin/go-amqp-amd64-linux src/main.go

# possible to build for windows
#GOOS=windows GOARCH=386 go build -o bin/go-amqp-386.exe src/main.go
# possible to build for macOS on different architectures
# 64-bit
#GOOS=darwin GOARCH=amd64 go build -o bin/go-amqp-amd64-darwin src/main.go
# 32-bit
#GOOS=darwin GOARCH=386 go build -o bin/go-amqp-386-darwin src/main.go
# possible to build for linux on different architectures
# 64-bit
#GOOS=linux GOARCH=amd64 go build -o bin/go-amqp-amd64-linux src/main.go
# 32-bit
#GOOS=linux GOARCH=386 go build -o bin/go-amqp-386-linux src/main.go
...
if [ "$docker" = true ]; then
    ...
    docker build --no-cache --build-arg VERSION=master -t valentinconan/go-amqp:master .
fi
